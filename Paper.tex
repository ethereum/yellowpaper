...
\item[to] The 160-bit address of the message call's recipient or $\varnothing$ for a contract creation transaction; formally $T_t$.
...
The address hash $T_\mathbf{t}$ is slightly different: it is either a 20-byte address hash or, in the case of being a contract-creation transaction (and thus formally equal to $\varnothing$), it is the RLP empty string and thus the member of $\mathbb{B}_0$:
\begin{equation}
T_t \in \begin{cases} \mathbb{B}_{0} & \text{if} \quad T_t = \varnothing \\
\mathbb{B}_{20} & \text{otherwise}\end{cases}
\end{equation}
...
\Theta_{3}(\boldsymbol{\sigma}_0, S(T), T_o, &\\ \quad\quad T_t, g, T_p, T_v, T_\mathbf{d}, 0) & \text{otherwise}
\end{cases}
\end{equation}
...
There are number of intrinsic parameters used when creating an account: sender ($s$), original transactor ($o$), available gas ($g$), gas price ($p$), endowment ($v$) together with an arbitrary length byte array, $\mathbf{i}$, the initialisation EVM code and finally the present depth of the message-call/contract-creation stack ($d$).
...
\begin{equation}
(\boldsymbol{\sigma}', g', A) \equiv \Lambda(\boldsymbol{\sigma}, s, o, g, p, v, \mathbf{i}, d)
\end{equation}

The address of the new account is defined as being the rightmost 160 bits of the Keccak hash of RLP encoding of the structure containing only the sender and the nonce. Thus we define the creation address function $A$:
\begin{eqnarray}
A(s, n) & \equiv & \mathcal{B}_{96..255}\Big(\mathtt{\tiny KEC}\Big(\mathtt{\tiny RLP}\big(\;(s, n)\;\big)\Big)\Big)
\end{eqnarray}
...
\end{equation}
\begin{equation}
\boldsymbol{\sigma}^*[A(s, \boldsymbol{\sigma}[s]_n - 1)] \equiv \big( 0, v + v', \mathtt{\tiny TRIE}(\varnothing), \mathtt{\tiny KEC}\big(()\big) \big)
\end{equation}

where $v'$ is the account's pre-existing value, in the event it was previously in existance:
...
\textit{TODO: rollback when $\mathbf{o} = \varnothing$ or is it sigma? clear this up, formally define and normalise into message-call and execution overview.}
\begin{equation}
...
\boldsymbol{\sigma}^{**} & \text{if} \quad a \notin \mathbf{s} \vee g^{**} < c \\
\begin{array}[b]{l}
...
No code is deposited in the state if the newly created account was suicided or if the gas does not cover the additional per-byte contract deposit fee.
...
I_s & \equiv & s \\
I_v & \equiv & v \\
\text{Let} \quad \mathtt{\tiny KEC}(I_\mathbf{b}) & = & \boldsymbol{\sigma}[r]_c
\end{eqnarray}
...
TODO: MUCH MORE DISCUSSION OF THIS
...
\textit{TODO: JUMP not onto valid JUMPDEST.}
...
\begin{equation}
S(T) \equiv \mathcal{B}_{96..255}\big(\mathtt{\tiny KEC}\big( \mathtt{\small ECDSARESTORE}(h(T), T_w, T_r, T_s) \big) \big)
\end{equation}
