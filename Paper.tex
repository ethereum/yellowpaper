...
\end{equation}
where $0$ denotes the zero element of $\mathbb{B}_{256}$, the operator $\cdot$ is the concatenation of binary data, $\mathtt{\tiny KEC}$ is the Keccak 256-bit hash function, $\mathcal{B}_{a..b}(X)$ evaluates to binary value containing the bits of indices in the range $[a, b]$ of the binary data.
...
When a transaction has a particular invalid signature $v = r = s = 0$, the sender of the transaction is $2^{256} - 1$.
\begin{equation}
\forall T: S(T) \equiv 2^{256} - 1 \quad \text{ where } (T_w, T_r, T_s) = (0, 0, 0).
\end{equation}
