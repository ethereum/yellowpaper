...
Fees (denominated in gas) are charged under three distinct circumstances, all three as prerequisite to the execution of an operation. The first and most common is the fee intrinsic to the computation of the operation. Most operations require a single gas fee to be paid for their execution; exceptions include \textsc{SSTORE}, \textsc{SLOAD}, \textsc{CALL}, \textsc{CREATE}, \textsc{BALANCE} and \textsc{SHA3}. Secondly, gas may be deducted in order to form the payment for a subordinate message call or contract creation; this forms part of the payment for \textsc{CREATE} and \textsc{CALL}. Finally, gas may be paid due to an increase in the usage of the memory.
...
\item SSTORE costs $d.G_{sstore}$ gas where:
\begin{itemize}
...
Additionally, when memory is accessed with MSTORE, MLOAD, RETURN, SHA3, CALLDATA or CALL, enlarge the memory so that all bytes now fit in it (memory must always be a whole number of 32-byte words). Suppose that the highest previously accessed memory index is $i$, and the new index is $i'$; an additional $a$ gas must be paid:

\begin{equation}
a \equiv \max(0, \ceil[\Bigg]{\frac{i'+1}{32}} - \ceil[\Bigg]{\frac{i+1}{32}}) G_{memory}
\end{equation}

Originally, memory being empty, the highest accessed index, $i$, is defined as $-1$.

memoryFee is proportional to greatest index that has ever been referenced in this instance and thus it's highly unlikely addresses will ever go above 32-bit. Whenever a higher memory index is used, the fee difference to take it to the higher usage from the original (lower) usage is charged. MEMSIZE is initially just 0. Notably, because \textsc{MSTORE} and \textsc{MLOAD} operate on word lengths, they increase the highest-accessed index to 32 greater than their target index.
