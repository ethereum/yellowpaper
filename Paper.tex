...
The other two components in the block are simply a list of uncle block headers (of the same format as above) and a list of the transaction information (as defined previously). Formally, we can refer to a block $B$:
\begin{equation}
...
H_t &\equiv& \mathtt{\small TRIE}(\{\forall i < \lVert B_\mathbf{T} \rVert: (i, B_\mathbf{T}[i])\}) & \wedge \\
...
TODO: Transaction manifest more than just transactions: $(T[i], s[i], g[i])$
...
The sum of the transaction gas limit and the gas utilised in this block prior must be no greater than the block's \textbf{gasLimit}, $H_l$.

% TODO: specify formally.
...
The given \textbf{gasUsed} and \textbf{minGasLimit} must correspond faithfully to the transactions listed. In the case of ${B_H}_m$, the minimum gas limit, all transactions included in the transaction manifest must have a gas price, ${T_0}_p$, that is at least this value:
\begin{equation}
\forall_{T \in B_\mathbf{T}} {T_0}_p \geq {B_H}_m
\end{equation}
...
As specified at the beginning of the present work, $\Pi$ is the state-transition function, which is defined in terms of $\Omega$, the block finalisation function and $\Upsilon$, the transaction-evaluation function, both now well-defined. We begin the definition by supposing we have a series of transactions $\mathbf{T}$.

\begin{equation}
\text{Let} \quad \mathbf{T} = (\mathbf{T}[0], \mathbf{T}[1], ...)
\end{equation}

These are the transactions that comprise the block, thus:
\begin{eqnarray}
\quad \quad B & \equiv & \Big(H, \big( \mathbf{M}[0], \mathbf{M}[1], ..., \mathbf{M}[\lVert \mathbf{T}\rVert - 1]\big), \mathbf{U}\Big) \\
\quad \quad \mathbf{M}[n] & \equiv & (\mathbf{T}[n], \mathbf{s}[n], \mathbf{g}[n])
\end{eqnarray}

Here, $\mathbf{s}$ and $\mathbf{g}$ are the corresponding sequences of states and cumulative gas used for each transaction. $\mathbf{s}$ is defined simply as the state resulting from applying the corrresponding transaction to the state resulting from the previous transaction (or the block's initial state in the case of the first such transaction):
\begin{equation}
\mathbf{s}[n] = \begin{cases} \Gamma(B) & \text{if} \quad n < 0 \\ \Upsilon(\mathbf{s}[n - 1], \mathbf{T}[n]) & \text{otherwise} \end{cases}
\end{equation}

In the case of $\mathbf{g}$, we take a similar approach defining each item as the gas used in evaluatng the corresponding transaction summed with the previous item (or zero, if it's the first), giving us a running total:
\begin{equation}
\mathbf{g}[n] = \begin{cases} 0 & \text{if} \quad n < 0 \\ \Upsilon^g(\mathbf{s}[n - 1], \mathbf{T}[n]) + \mathbf{g}[n-1] & \text{otherwise} \end{cases}
\end{equation}
...
\begin{equation}
\Pi(\boldsymbol{\sigma}, B) \equiv \Omega(B, \ell(B_\mathbf{T})_1)
\end{equation}
