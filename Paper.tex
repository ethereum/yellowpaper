...
The block in Ethereum is the collection of relevant pieces of information (known as the block \textit{header}), $H$, together with information corresponding to the comprised transactions, $\mathbf{T}$, and a set of other block headers $\mathbf{U}$ that are known to have a parent equal to the present block's parent's parent (such blocks are known as \textit{aunts}). The block header contains several pieces of information:
...
\item[auntsHash] The Keccak 256-bit hash of the aunts list portion of this block; formally $H_u$.
...
The other two components in the block are simply a list of aunt block headers (of the same format as above) and a series of the transactions. Formally, we can refer to a block $B$:
\begin{equation}
...
We can assert a block's validity if and only if it satisfies several conditions: it must be internally consistent with the aunt and transaction block hashes and the given transactions $B_\mathbf{T}$ (as specified in sec \ref{ch:finalisation}), when executed in order on the base state $\boldsymbol{\sigma}$ (derived from the final state of the parent block), result in a new state of the identity $H_r$:
\begin{equation}
\begin{array}[t]{lclc}
...
As such given a block $B$, $B_t$ is its total difficulty, $B'$ is its parent block, $B_d$ is its difficulty and $B_\mathbf{U}$ is its set of aunt blocks.
...
\begin{enumerate}
\item Validate (or, if mining, determine) aunts;
\item validate (or, if mining, determine) transactions;
\item apply rewards;
\item verify (or, if mining, compute a valid) state and nonce.
\end{enumerate}

\subsection{Aunt Validation}

The validation of aunt headers means nothing more than verifying that each aunt header is both a valid header and satisfies the relation of $N$th-generation aunt to the present block where $N \leq 6$. Formally:
\begin{equation}
...
The application of rewards to a block involves raising the balance of the accounts of the coinbase address of the block and each aunt by a certain amount. We raise the block's coinbase account by $R_b$; for each aunt, we raise the block's coinbase by an additional $\frac{1}{32}$ of the block reward and the coinbase of the aunt by $\frac{15}{16}$ of the reward. Formally we define the function $\Omega$:
\begin{eqnarray}
...
If there are collisions of the coinbase addresses between aunts and the block (i.e. two aunts with the same coinbase address or an aunt with the same coinbase address as the present block), additions are applied cumulatively.
...
Where $0_{256}$ refers to the parent hash, a 256-bit hash which is all zeroes; $0_{160}$ refers to the coinbase address, a 160-bit hash which is all zeroes; $2^{22}$ refers to the difficulty; 0 refers to the timestamp (the Unix epoch); the transaction trie root and extradata are both $0$, being equivalent to the empty byte array. The sequences of both aunts and transactions are empty and represented by $()$. $\mathtt{\tiny KEC}\big( (42) \big)$ refers to the Keccak hash of a byte array of length one whose first and only byte is of value 42. $\mathtt{\tiny KEC}\big(\mathtt{\tiny RLP}\big( () \big)\big)$ value refers to the hash of the aunt lists in RLP, both empty lists.
