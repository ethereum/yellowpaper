...
\mathbf{T} & \equiv & [ T_0, T_1, ... ] \\
\Pi(\mathbb{S}, \mathbf{T}) & \equiv & \Omega(\Upsilon(\Upsilon(\mathbb{S}, T_0), T_1) ...)
\end{eqnarray}

Where $\Omega$ is the block-finalisation state transition function (a function that rewards a nominated party); $\mathbf{T}$ is the series of transactions included in this block; and $\Pi$ is the block-level state-transition function.
...
\item[extraData] An arbitrary byte array containing data relavant to this block. Generally 256-bit or less, but could be much larger in the genesis block; formally $H_x$.
...
\subsection{Transaction Execution} \label{ch:transactions}
...
\item the sender account balance contains at least the cost, $v_0$, (in wei) required in up-front payment;
\end{enumerate}
...
After the message call or contract creation is processed, the state is finalised by refunding $g'$, the remaining gas, to the sender at the original rate. So we define the final state $\mathbb{S}'$ in terms of the provisional state $\mathbb{S}_P$.
...
\subsection{Contract Creation} \label{ch:create}
...
a' & \equiv & \mathcal{B}_{96..255}(\mathcal{S}(\mathcal{R}([s, o]))) \\
a & \equiv & \arg \min_x : x \geqslant a' \wedge \mathbb{S}_0[x] = \varnothing
\end{eqnarray}
...
\subsection{Message Call} \label{ch:call}
...
&&&& $\mathbb{M}'_T[0] \equiv \mathcal{I}_\mathbf{d}[ \mathbb{M}_S[0] \dots (\mathbb{M}_S[0] + 31) ]$ \\
...
&&&& $\mathbb{M}'_T[0] \equiv \lVert \mathcal{I}_\mathbf{d} \rVert$ \\
&&&& This pertains to the input data passed with the message call instruction or transaction. \\
\midrule
