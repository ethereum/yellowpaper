...
% info on the above command: https://tex.stackexchange.com/posts/10419/revisions
...
&&&& This is the sender of the original transaction; it is never an account with non-empty \\
&&&& associated code. \\
\midrule
...
&&&& (the maximum call depth limit is reached) or $\boldsymbol{\mu}_\mathbf{s}[0] > \boldsymbol{\sigma}[I_a]_b$ (the balance of the caller is  \\
&&&& too low to fulfil the value transfer); and otherwise $x=A(I_a, \boldsymbol{\sigma}[I_a]_n)$, the address of the  \\
