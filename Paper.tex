...
The final state, $\boldsymbol{\sigma}'$, is reached after deleting all accounts that appear in the self-destruct list:
\begin{eqnarray}
...
The execution model defines the function $\Xi$, which can compute the resultant state $\boldsymbol{\sigma}'$, the remaining gas $g'$, the self-destruct list $\mathbf{s}$, the log series $\mathbf{l}$, the refunds $r$ and the resultant output, $\mathbf{o}$, given these definitions:
\begin{equation}
...
In general, we assume the memory, self-destruct list and system state don't change:
\begin{eqnarray}
