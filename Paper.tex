...
\title{Ethereum: A Secure Decentralised Generalised Transaction Ledger \\ {\smaller \textbf{Final Draft - WiP}}}
\author{
    Dr. Gavin Wood\\
...
If the \textbf{codeHash} field is the Keccak hash of the empty string, i.e. $\boldsymbol{\sigma}[a]_c = \texttt{\small KEC}\big(()\big)$, then the node represents a simple account, sometimes referred to as a ``non-contract'' account.
...
\begin{description}
\item[parentHash] The Keccak 256-bit hash of the parent block, in its entirety; formally $H_p$.
...
\item[transactionsTrie] The Keccak 256-bit hash of the root node of the trie structure populated with each transaction in the transactions list portion of the block; formally $H_t$.
...
\begin{equation}
\begin{array}[t]{lclc}
H_u &\equiv& \mathtt{\small Keccak}(\mathtt{\small RLP}(L_H^*(B_\mathbf{U}))) & \wedge \\
...
\begin{eqnarray}
\quad L_H(H) & \equiv & (\begin{array}[t]{l}H_p, H_u, H_b, H_r, H_t, H_d,\\ H_i, H_l, H_u, H_s, H_x, H_n \; )\end{array} \\
...
\begin{equation}
P(H) \equiv B': \mathtt{\tiny Keccak}(\mathtt{\tiny RLP}(B')) = H_p
\end{equation}
...
The address of the new account is defined as being the rightmost 160 bits of the Keccak hash of RLP encoding of the structure containing only the sender and the nonce. In the unlikely event that the address is already in use, it is treated as a big-endian integer and incremented by one until an unused address is arrived at. Thus we define the creation address function $A$:
\begin{eqnarray}
A(s, n) & \equiv & \mathcal{B}_{96..255}\Big(\mathtt{\tiny Keccak}\Big(\mathtt{\tiny RLP}\big(\;(s, n)\;\big)\Big)\Big)
\end{eqnarray}

where $\mathtt{\tiny Keccak}$ is the Keccak 256-bit hash function, $\mathtt{\tiny RLP}$ is the RLP encoding function, $\mathcal{B}_{a..b}(X)$ evaluates to binary value containing the bits of indices in the range $[a, b]$ of the binary data $X$ and $\boldsymbol{\sigma}[x]$ is the address state of $x$ or $\varnothing$ if none exists. Note, as defined directly, we use one fewer than the sender's nonce value; we assert that we have incremented the sender account's nonce prior to this call, and so the value used is the sender's nonce at the beginning of the responsible transaction or VM operation.
...
\end{equation}
\begin{equation}
\boldsymbol{\sigma}^*[A(s, \boldsymbol{\sigma}[s]_n - 1)] \equiv \big( 0, v, \mathtt{\tiny TRIE}(\varnothing), \mathtt{\tiny Keccak}\big(()\big) \big)
\end{equation}

where $a$ is the address of the new account, as defined above.
% It is asserted that the state database will also change such that it defines the pair $(\mathtt{\tiny Keccak}(\mathbf{b}), \mathbf{b})$.
...
\begin{equation}
\begin{cases}
\boldsymbol{\sigma}_1[r] \equiv (v, 0, \mathtt{\tiny KEC}(()), \mathtt{\tiny TRIE}(())) & \text{if} \quad \boldsymbol{\sigma}[r] = \varnothing \\
...
I_s & \equiv & s \\
I_v & \equiv & v \\
\text{Let} \quad \mathtt{\tiny Keccak}(I_\mathbf{b}) & = & \boldsymbol{\sigma}[r]_c
\end{eqnarray}

It is assumed that the client will have stored the pair $(\mathtt{\tiny Keccak}(I_\mathbf{b}), I_\mathbf{b})$ at some point prior in order to make the determination of $I_\mathbf{b}$ feasible.
...
%
%\begin{itemize}
%\item {\small Keccak} costs $G_{sha3}$ gas
...
%\end{itemize}
%
%Additionally, when memory is accessed with {\small MSTORE}, {\small MSTORE8}, {\small MLOAD}, {\small CALLDATACOPY}, {\small CODECOPY}, {\small RETURN}, {\small Keccak}, {\small CREATE} or {\small CALL}, the memory should be enlarged to the smallest multiple of words such that all addressed bytes now fit in it.
...
\begin{eqnarray}
\Xi(\boldsymbol{\sigma}, g, I) & \equiv & X_{0,1,2,4}\big((\boldsymbol{\sigma}, \boldsymbol{\mu}, \varnothing, I)\big) \\
...
\big(\boldsymbol{\sigma}, \boldsymbol{\mu}, \varnothing, I, ()\big) & \text{if} \quad Z(\boldsymbol{\sigma}, \boldsymbol{\mu}, I)\\
...
\boldsymbol{\sigma}'[{B_H}_b]_b & = & \boldsymbol{\sigma}[{B_H}_b]_b + (1 + \frac{|B_\mathbf{U}|}{32})R_b \\
\forall_{U \in B_\mathbf{U}} \quad \boldsymbol{\sigma}'[U_b]_b & = & \boldsymbol{\sigma}[U_b]_b + \frac{15}{16}R_b
\end{eqnarray}
...
\begin{equation}
\mathtt{PoW}(H, n) \equiv \mathtt{BE}(\mathtt{\tiny Keccak}(\mathtt{\tiny Keccak}(\mathtt{\tiny RLP}(H_{\hcancel{n}})) \circ n))
\end{equation}

where:
$\mathtt{\tiny RLP}(H_{\hcancel{n}})$ is the RLP encoding of the block header $H$, not including the final nonce component;
$\mathtt{\tiny Keccak}$ is the Keccak hash function accepting an arbitrary length series of bytes and evaluating to a series of 32 bytes (i.e. 256-bit);
...
\item[LLL] The Lisp-like Low-level Language, a human-writable language used for authoring simple contracts and general low-level language toolkit for transpiling to.
...
\end{cases} \\
\mathtt{\tiny BE}(x) & \equiv & (b_0, b_1, ...): b_0 \neq 0 \wedge \sum_{n = 0}^{\lVert \mathbf{b} \rVert - 1} b_n256^{\lVert \mathbf{b} \rVert - 1 - n} \\
...
\begin{equation}
A(p_r) = \mathcal{B}_{96..255}\big(\mathtt{\tiny Keccak}\big( \mathtt{\small ECDSAPUBKEY}(p_r) \big) \big)
\end{equation}
...
\end{cases} \\
h(T) & \equiv & \mathtt{\small Keccak}( L_S(T) )
\end{eqnarray}
...
\begin{equation}
S(T) \equiv \mathcal{B}_{96..255}\big(\mathtt{\tiny Keccak}\big( \mathtt{\small ECDSARESTORE}(h(T), T_w, T_r, T_s) \big) \big)
\end{equation}
...
$R_{sclear}$ & 150 & Refund given (added into refund counter) when the storage value is set to zero from non-zero. \\
...
C_\text{\tiny EXP}(\boldsymbol{\mu}) & \text{if} \quad w = \text{\small EXP} \\
...
\midrule
0x5b & {\small JUMPDEST} & 0 & 1 & Mark a valid destination for jumps. \\
&&&& TODO \\
\bottomrule
\end{tabular*}
...
&&&& $(\boldsymbol{\sigma}', \boldsymbol{\mu}'_g, \mathbf{s}^+) \equiv \begin{cases}\Lambda(\boldsymbol{\sigma}^*, I_a, I_o, \boldsymbol{\mu}_g, I_p, \boldsymbol{\mu}_\mathbf{s}[0], \mathbf{i}) & \text{when} \quad \boldsymbol{\mu}_\mathbf{s}[0] \leqslant \boldsymbol{\sigma}[I_a]_b \\ \big(\boldsymbol{\sigma}, \boldsymbol{\mu}_g, \varnothing\big) & \text{otherwise} \end{cases}$ \\
...
&&&& Same as CALL to this account, except that the code is exchanged for $\boldsymbol{\sigma}[\boldsymbol{\mu}_s[1]]_c$. \\
&&&& TODO \\
\midrule
...
\begin{equation}
\big( \big( 0_{256}, \mathtt{\tiny Keccak}\big(\mathtt{\tiny RLP}\big( () \big)\big), 0_{160}, stateRoot, 0, 2^{22}, 0, 0, 1000000, 0, 0, 0, \mathtt{\tiny Keccak}\big( (42) \big) \big), (), () \big)
\end{equation}

Where $0_{256}$ refers to the parent hash, a 256-bit hash which is all zeroes; $0_{160}$ refers to the coinbase address, a 160-bit hash which is all zeroes; $2^{22}$ refers to the difficulty; 0 refers to the timestamp (the Unix epoch); the transaction trie root and extradata are both $0$, being equivalent to the empty byte array. The sequences of both uncles and transactions are empty and represented by $()$. $\mathtt{\tiny Keccak}\big( (42) \big)$ refers to the Keccak hash of a byte array of length one whose first and only byte is of value 42. $\mathtt{\tiny Keccak}\big(\mathtt{\tiny RLP}\big( () \big)\big)$ value refers to the hash of the uncle lists in RLP, both empty lists.
