...
This states that the execution is in an exceptional halting state if there is insufficient gas or if there are insufficient stack items. The astute reader will realise that this implies that no instruction can, through its execution, cause an exceptional halt.
...
&&&& $\mathbb{M}'_T[0] \equiv \begin{cases}0 & \text{if} \quad \mathbb{M}_S[1] = 0\\ \lfloor\mathbb{M}_S[0] \div \mathbb{M}_S[1]\rfloor & \text{otherwise}\end{cases}$  \\
\midrule
...
&&&& $\mathbb{M}'_T[0] \equiv \begin{cases}0 & \text{if} \quad \mathbb{M}_S[1] = 0\\ \mathbb{M}_S[0] \bmod \mathbb{M}_S[1] & \text{otherwise}\end{cases}$  \\
\midrule
