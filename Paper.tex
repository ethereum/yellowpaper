...
Incorruptability of judgement. Transparency. Open state. User can be guaranteed that backend will do what front-end says it will. Parties can analyse and be wholly confident in outcomes given events.
...
\begin{equation}
P(H) \equiv B': \mathcal{S}(\mathcal{R}(B')) = H_p
\end{equation}
...
a' & = & \mathcal{B}_{96..255}(\mathcal{S}(\mathcal{R}([s, n])))
\end{eqnarray}

where $\mathcal{S}$ is the SHA3 256-bit hash function, $\mathcal{R}$ is the RLP encoding function, $\mathcal{B}_{a..b}(X)$ evaluates to binary value containing the bits of indices in the range $[a, b]$ of the binary data $X$ and $\mathbb{S}_0[x]$ is the address state of $x$ or $\varnothing$ if none exists. Note we use one fewer than the sender's nonce value; we assert that we have incremented the sender account's nonce prior to this call, and so the value used is the sender's nonce at the beginning of the responsible transaction or VM operation.
...
\mathbb{S}^*[A(s, \mathbb{S}[s]_{nonce})] & \equiv & ( 0, v, \varnothing, \mathcal{S}(\mathbf{b}) )
\end{eqnarray}
...
$\langle 0_{256}, \mathcal{S}(\mathcal{R}([])), 0_{160}, 0_{256}, \mathcal{S}(\mathcal{R}([])), 2^{22}, 0, [], 42 \rangle$
