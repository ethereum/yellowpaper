...
The PoW-function can be defined as an array with the compressed mix as its first item and the Keccak-256 hash of the concatenation of the compressed mix with the seed hash as the second item:
\begin{equation}
 \mathtt{PoW}(H, H_n, \mathbf{d}) = \lbrace \mathbf{m}_c(H, H_n, \mathbf{d}), \texttt{KEC}(\mathbf{s}_h(H, H_n) + \mathbf{m}_c(H, H_n, \mathbf{d})) \rbrace
\end{equation}
The compressed mix $\mathbf{m}_c$ is obtained as follows:
\begin{equation}
 \mathbf{m}_c(H, H_n, \mathbf{d}) = E_{compress}(E_{accesses}(\mathbf{d}, \sum_{i = 0}^{n_{mix}} \mathbf{s}_h(H, H_n), \mathbf{s}_h(H, H_n), -1), -4)
\end{equation}
...
With $\mathbf{h}$ being the hash of the header without the nonce, and $E_{revert}(\mathbf{n})$ the reverted bytes sequence of the nonce $\mathbf{n}$:
\begin{equation}
...
E_{accesses}(E_{mixdataset}(\mathbf{d}, \mathbf{m}), \mathbf{s}, i + 1) & \text{otherwise}
\end{cases}
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
 E_{newdata}(\mathbf{d}, \mathbf{m}, \mathbf{s}, i)[j] = \mathbf{d}[E_\text{\tiny FNV}(i \oplus \mathbf{s}[0], \mathbf{m}[i \mod \left\lfloor\frac{J_{mixbytes}}{J_{wordbytes}}\right\rfloor) \mod \left\lfloor\frac{d_{size} / J_{hashbytes}}{n_{mix}}\right\rfloor \cdot n_{mix} + j]
\end{equation}
