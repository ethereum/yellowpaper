...
Let $G_T$ be the multiplicative abelian group underlying $F_{q^{12}}$. It is known that a non-degenerate bilinear map $e : G_1\times G_2 \to G_T$ exists.
This bilinear map is a type three pairing. Let $P_T = e(P_1, P_2)$, $a$ be a set of $k$ points in $G_1$, and $b$ be a set of $k$ points that are in $G_2$. It follows from the definition of a pairing that the following are equivalent
...
\end{eqnarray}
Thus the pairing check provides a method to verify \ref{eq:pairing-check}.
