...
\begin{equation}
(\boldsymbol{\sigma}_P, g', A, e) \equiv \begin{cases}
...
Note we use $\Theta_{3}$ and $\Lambda_{3}$ to denote the fact that only the first three components of the function's value are taken; the final represents the message-call's output value (a byte array) and is unused in the context of transaction evaluation.
...
\begin{equation}
(\boldsymbol{\sigma}', g', A, e) \equiv \Lambda(\boldsymbol{\sigma}, s, o, g, p, v, \mathbf{i}, e)
\end{equation}
...
If such an exception does not occur, then the remaining gas is refunded to the originator and the now-altered state is allowed to persist. Thus formally, we may specify the resultant state, gas, substate and exception flag as $(\boldsymbol{\sigma}', g', A, e)$ where:
...
\end{cases} \\
\quad e &\equiv  \begin{cases}
\top & \text{if} \quad \boldsymbol{\sigma}^{**} = \varnothing \\
\bot & \text{otherwise}
\end{cases}
\end{align}
...
\begin{equation}
(\boldsymbol{\sigma}', g', A, \mathbf{o}, e) \equiv \Theta(\boldsymbol{\sigma}, s, o, r, c, g, p, v, \tilde{v}, \mathbf{d}, e)
\end{equation}
...
&&&& $e = \top$ or $I_e = 1024$ (the maximum call depth limit is reached) or $\boldsymbol{\mu}_\mathbf{s}[0] > \boldsymbol{\sigma}[I_a]_b$ \\
&&&& (balance of the caller is too low to fulfil the value transfer); and otherwise $x=A(I_a, \boldsymbol{\sigma}[I_a]_n)$,\\
&&&& the address of the newly created account, otherwise. \\
...
&&&& where $x=0$ if the code execution for this operation failed due to an exceptional halting \\
&&&& $e = \top$ or if $\boldsymbol{\mu}_\mathbf{s}[2] > \boldsymbol{\sigma}[I_a]_b$ (not enough funds) or $I_e = 1024$ (call depth limit reached); \\
&&&& $x=1$ otherwise. \\
...
&&&& $(\boldsymbol{\sigma}', g', A^+, \mathbf{o}, e) \equiv \begin{cases}\begin{array}{l}\Theta(\boldsymbol{\sigma}^*, I_s, I_o, I_a, t,\\\quad \boldsymbol{\mu}_\mathbf{s}[0], I_p, 0, I_v, \mathbf{i}, I_e + 1)\end{array} & \text{if} \quad I_v \leqslant \boldsymbol{\sigma}[I_a]_b \;\wedge\; I_e < 1024 \\ (\boldsymbol{\sigma}, g, \varnothing, (), \top) & \text{otherwise} \end{cases}$ \\
