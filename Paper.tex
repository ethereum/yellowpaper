...
\item[gasLimit]\linkdest{tx_gas_limit_T__g}{} A scalar value equal to the maximum amount of gas that should be used in executing this transaction. This is paid up-front, before any computation is done and may not be increased later; formally $T_{\mathrm{\mathrm{g}}}$.
...
(T_{\mathrm{n}}, T_{\mathrm{p}}, T_{\mathrm{\mathrm{g}}}, T_{\mathrm{t}}, T_{\mathrm{v}}, T_{\mathbf{i}}, T_{\mathrm{w}}, T_{\mathrm{r}}, T_{\mathrm{s}}) & \text{if} \; T_{\mathrm{t}} = \varnothing\\
(T_{\mathrm{n}}, T_{\mathrm{p}}, T_{\mathrm{\mathrm{g}}}, T_{\mathrm{t}}, T_{\mathrm{v}}, T_{\mathbf{d}}, T_{\mathrm{w}}, T_{\mathrm{r}}, T_{\mathrm{s}}) & \text{otherwise}
\end{cases}
\end{equation}
...
T_{\mathrm{\mathrm{g}}} \in \mathbb{P}_{256} & \wedge & T_{\mathrm{w}} \in \mathbb{P}_5 & \wedge & T_{\mathrm{r}} \in \mathbb{P}_{256} & \wedge \\
...
\item[gasUsed]\linkdest{block_gas_used_H__g}{}\linkdest{H__g} A scalar value equal to the total gas used in transactions in this block; formally $H_{\mathrm{\mathrm{g}}}$.
...
\begin{eqnarray}
\quad L_{H}(H) & \equiv & (\begin{array}[t]{l}H_{\mathrm{p}}, H_{\mathrm{o}}, H_{\mathrm{c}}, H_{\mathrm{r}}, H_{\mathrm{t}}, H_{\mathrm{e}}, H_{\mathrm{b}}, H_{\mathrm{d}},\\ H_{\mathrm{i}}, H_{\mathrm{l}}, H_{\mathrm{\mathrm{g}}}, H_{\mathrm{s}}, H_{\mathrm{x}}, H_{\mathrm{m}}, H_{\mathrm{n}} \; )\end{array} \\
...
\hyperlink{block_gas_limit_H__l}{H_{\mathrm{l}}} \in \mathbb{P} & \wedge & H_{\mathrm{\mathrm{g}}} \in \mathbb{P} & \wedge & \hyperlink{block_timestamp_H__s}{H_{\mathrm{s}}} \in \mathbb{P}_{256} & \wedge \\
...
& & H_{\mathrm{\mathrm{g}}} \le H_{\mathrm{l}}  \quad \wedge \\
& & H_{\mathrm{l}} < {P(H)_{H}}_{\mathrm{l}} + \left\lfloor\frac{{P(H)_{H}}_{\mathrm{l}}}{1024}\right\rfloor  \quad \wedge \\
& & H_{\mathrm{l}} > {P(H)_{H}}_{\mathrm{l}} - \left\lfloor\frac{{P(H)_{H}}_{\mathrm{l}}}{1024}\right\rfloor  \quad \wedge \\
...
\item the gas limit is no smaller than the intrinsic gas, $\mathrm{\mathrm{g}}_0$, used by the transaction; and
...
\begin{align}
\mathrm{\mathrm{g}}_0 \equiv {} & \sum_{i \in T_{\mathbf{i}}, T_{\mathbf{d}}} \begin{cases} \hyperlink{G__txdatazero}{G_{\mathrm{txdatazero}}} & \text{if} \quad i = 0 \\ \hyperlink{G__txdatanonzero}{G_{\mathrm{txdatanonzero}}} & \text{otherwise} \end{cases} \\
...
\begin{equation}
v_0 \equiv \hyperlink{tx_gas_limit_T__g}{T_{\mathrm{\mathrm{g}}}} \hyperlink{tx_gas_price_T__p}{T_{\mathrm{p}}} + \hyperlink{tx_value_T__v}{T_{\mathrm{v}}}
\end{equation}
...
\mathrm{\mathrm{g}}_0 & \leqslant & T_{\mathrm{\mathrm{g}}} \quad \wedge \\
v_0 & \leqslant & \boldsymbol{\sigma}[S(T)]_{\mathrm{b}} \quad \wedge \\
T_{\mathrm{\mathrm{g}}} & \leqslant & {B_{H}}_{\mathrm{l}} - \hyperlink{ell}{\ell}(B_{\mathbf{R}})_{\mathrm{u}}
\end{array}
\end{equation}

Note the final condition; the sum of the transaction's gas limit, $T_{\mathrm{\mathrm{g}}}$, and the gas utilised in this block prior, given by $\hyperlink{ell}{\ell}(B_{\mathbf{R}})_{\mathrm{u}}$, must be no greater than the block's \textbf{gasLimit}, ${B_{H}}_{\mathrm{l}}$.

The execution of a valid transaction begins with an irrevocable change made to the state: the \hyperlink{account_nonce}{nonce of the account of the sender}, $S(T)$, is incremented by one and the balance is reduced by part of the up-front cost, $T_{\mathrm{\mathrm{g}}}T_{\mathrm{p}}$. The gas available for the proceeding computation, $\mathrm{g}$, is defined as $T_{\mathrm{\mathrm{g}}} - \mathrm{\mathrm{g}}_0$. The computation, whether contract creation or a message call, results in an eventual state (which may legally be equivalent to the current state), the change to which is deterministic and never invalid: there can be no invalid transactions from this point.

We define the checkpoint state $\boldsymbol{\sigma}_0$:
\begin{eqnarray}
\boldsymbol{\sigma}_0 & \equiv & \boldsymbol{\sigma} \quad \text{except:} \\
\boldsymbol{\sigma}_0[S(T)]_{\mathrm{b}} & \equiv & \boldsymbol{\sigma}[S(T)]_{\mathrm{b}} - T_{\mathrm{\mathrm{g}}} T_{\mathrm{p}} \\
\boldsymbol{\sigma}_0[S(T)]_{\mathrm{n}} & \equiv & \boldsymbol{\sigma}[S(T)]_{\mathrm{n}} + 1
\end{eqnarray}

Evaluating $\boldsymbol{\sigma}_{P}$ from $\boldsymbol{\sigma}_0$ depends on the transaction type; either contract creation or message call; we define the tuple of post-execution provisional state $\boldsymbol{\sigma}_{P}$, remaining gas $\mathrm{g}'$, substate $A$ and status code $\mathrm{z}$:
\begin{equation}
(\boldsymbol{\sigma}_{P}, \mathrm{g}', A, z) \equiv \begin{cases}
\Lambda_{4}(\boldsymbol{\sigma}_0, S(T), T_{\mathrm{o}}, &\\ \quad\quad \mathrm{g}, T_{\mathrm{p}}, T_{\mathrm{v}}, T_{\mathbf{i}}, 0, \top) & \text{if} \quad T_{\mathrm{t}} = \varnothing \\
\Theta_{4}(\boldsymbol{\sigma}_0, S(T), T_{\mathrm{o}}, &\\ \quad\quad T_{\mathrm{t}}, T_{\mathrm{t}}, \mathrm{g}, T_{\mathrm{p}}, T_{\mathrm{v}}, T_{\mathrm{v}}, T_{\mathbf{d}}, 0, \top) & \text{otherwise}
\end{cases}
\end{equation}

where $\mathrm{g}$ is the amount of gas remaining after deducting the basic amount required to pay for the existence of the transaction:
\begin{equation}
\mathrm{g} \equiv T_{\mathrm{\mathrm{g}}} - g_0
\end{equation}
...
After the message call or contract creation is processed, the state is finalised by determining the amount to be refunded, $\mathrm{g}^*$ from the remaining gas, $\mathrm{g}'$, plus some allowance from the refund counter, to the sender at the original rate.
\begin{equation}
\mathrm{g}^* \equiv \mathrm{g}' + \min \{ \Big\lfloor \dfrac{T_{\mathrm{\mathrm{g}}} - \mathrm{g}'}{2} \Big\rfloor, \hyperlink{refund_balance_defn_words_A__r}{A_{\mathrm{r}}} \}
\end{equation}

The total refundable amount is the legitimately remaining gas $\mathrm{g}'$, added to \hyperlink{refund_balance_defn_words_A__r}{$A_{\mathrm{r}}$}, with the latter component being capped up to a maximum of half (rounded down) of the total amount used $T_{\mathrm{\mathrm{g}}} - \mathrm{g}'$.
...
\boldsymbol{\sigma}^*[S(T)]_{\mathrm{b}} & \equiv & \boldsymbol{\sigma}_{P}[S(T)]_{\mathrm{b}} + \mathrm{g}^* T_{\mathrm{p}} \\
\boldsymbol{\sigma}^*[m]_{\mathrm{b}} & \equiv & \boldsymbol{\sigma}_{P}[m]_{\mathrm{b}} + (T_{\mathrm{\mathrm{g}}} - \mathrm{g}^*) T_{\mathrm{p}} \\
...
If the initialization code completes successfully, a final contract-creation cost is paid, the code-deposit cost, $\mathrm{c}$, proportional to the size of the created contract's code:
\begin{equation}
c \equiv G_{codedeposit} \times |\mathbf{o}|
\end{equation}

If there is not enough gas remaining to pay this, \ie $\mathrm{g}^{**} < c$, then we also declare an out-of-gas exception.
...
If such an exception does not occur, then the remaining gas is refunded to the originator and the now-altered state is allowed to persist. Thus formally, we may specify the resultant state, gas, substate and status code as $(\boldsymbol{\sigma}', \mathrm{g}', A, z)$ where:

\begin{align}
\quad \mathrm{g}' &\equiv \begin{cases}
0 & \text{if} \quad F \\
\mathrm{g}^{**} - c & \text{otherwise} \\
\end{cases} \\
...
0 & \text{if} \quad \boldsymbol{\sigma}^{**} = \varnothing \lor \mathrm{g}^{**} < c \\
1 & \text{otherwise}
\end{cases} \\
\nonumber \text{where} \\
F &\equiv \big((\boldsymbol{\sigma}^{**} = \varnothing \ \wedge\ \mathbf{o} = \varnothing) \vee\  \mathrm{g}^{**} < c \ \vee\  |\mathbf{o}| > 24576\big)
\end{align}
...
In the case of executing a message call, several parameters are required: sender ($s$), transaction originator ($o$), recipient ($r$), the account whose code is to be executed ($\mathrm{c}$, usually the same as recipient), available gas ($\mathrm{g}$), value ($v$) and gas price ($p$) together with an arbitrary length byte array, $\mathbf{d}$, the input data of the call and finally the present depth of the message-call/contract-creation stack ($\mathrm{e}$).
...
\begin{equation}
(\boldsymbol{\sigma}', \mathrm{g}', A, z, \mathbf{o}) \equiv \Theta(\boldsymbol{\sigma}, s, o, r, c, \mathrm{g}, p, v, \tilde{v}, \mathbf{d}, e, w)
\end{equation}
...
\end{cases} \\
\mathrm{g}' & \equiv & \begin{cases}
0 & \text{if} \quad \boldsymbol{\sigma}^{**} = \varnothing \ \wedge \ \mathbf{o} = \varnothing \\
\mathrm{g}^{**} & \text{otherwise}
\end{cases} \\ \nonumber
z & \equiv & \begin{cases}
0 & \text{if} \quad \boldsymbol{\sigma}^{**} = \varnothing \\
1 & \text{otherwise}
\end{cases} \\
\qquad (\boldsymbol{\sigma}^{**}, \mathrm{g}^{**}, A, \mathbf{o}) & \equiv & \begin{cases}
\Xi_{\mathtt{ECREC}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 1 \\
\Xi_{\mathtt{SHA256}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 2 \\
\Xi_{\mathtt{RIP160}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 3 \\
\Xi_{\mathtt{ID}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 4 \\
\Xi_{\mathtt{EXPMOD}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 5 \\
\Xi_{\mathtt{BN\_ADD}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 6 \\
\Xi_{\mathtt{BN\_MUL}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 7 \\
\Xi_{\mathtt{SNARKV}}(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{if} \quad r = 8 \\
\Xi(\boldsymbol{\sigma}_1, \mathrm{g}, I, \mathbf{t}) & \text{otherwise} \end{cases} \\
I_{\mathrm{a}} & \equiv & r \\
I_{\mathrm{o}} & \equiv & o \\
I_{\mathrm{p}} & \equiv & p \\
...
In addition to the system state $\boldsymbol{\sigma}$, and the remaining gas for computation $\mathrm{g}$, there are several pieces of important information used in the execution environment that the execution agent must provide; these are contained in the tuple $I$:
...
The execution model defines the function $\Xi$, which can compute the resultant state $\boldsymbol{\sigma}'$, the remaining gas $\mathrm{g}'$, the accrued substate $A$ and the resultant output, $\mathbf{o}$, given these definitions. For the present context, we will define it as:
\begin{equation}
(\boldsymbol{\sigma}', \mathrm{g}', A, \mathbf{o}) \equiv \Xi(\boldsymbol{\sigma}, \mathrm{g}, I)
\end{equation}
...
\begin{eqnarray}
\Xi(\boldsymbol{\sigma}, \mathrm{g}, I, T) & \equiv & (\boldsymbol{\sigma}'\!, \boldsymbol{\mu}'_{\mathrm{\mathrm{g}}}, A, \mathbf{o}) \\
(\boldsymbol{\sigma}', \boldsymbol{\mu}'\!, A, ..., \mathbf{o}) & \equiv & X\big((\boldsymbol{\sigma}, \boldsymbol{\mu}, A^0\!, I)\big) \\
\boldsymbol{\mu}_{\mathrm{\mathrm{g}}} & \equiv & \mathrm{g} \\
...
\boldsymbol{\mu}'_{\mathrm{\mathrm{g}}} & \equiv & \boldsymbol{\mu}_{\mathrm{\mathrm{g}}} - C(\boldsymbol{\sigma}, \boldsymbol{\mu}, I)
\end{eqnarray}

Note that, when we evaluate $\Xi$, we drop the fourth element $I'$ and extract the remaining gas $\boldsymbol{\mu}'_{\mathrm{\mathrm{g}}}$ from the resultant machine state $\boldsymbol{\mu}'$.
...
The machine state $\boldsymbol{\mu}$ is defined as the tuple $(\mathrm{g}, pc, \mathbf{m}, i, \mathbf{s})$ which are the gas available, the program counter $pc \in \mathbb{P}_{256}$ , the memory contents, the active number of words in memory (counting continuously from position 0), and the stack contents. The memory contents $\boldsymbol{\mu}_{\mathbf{m}}$ are a series of zeroes of size $2^{256}$.
...
\begin{array}[t]{l}
\boldsymbol{\mu}_{\mathrm{\mathrm{g}}} < C(\boldsymbol{\sigma}, \boldsymbol{\mu}, I) \quad \vee \\
...
\item apply rewards;
\item verify (or, if mining, compute a valid) state and \hyperlink{block nonce}{block nonce}.
\end{enumerate}
...
%where $s[i]$ equals the root of the state trie immediately after the execution of the transaction $B_{\mathbf{T}}[i]$, and $\mathrm{g}[i]$ the total gas used immediately after said transaction.

The given \textbf{gasUsed} must correspond faithfully to the transactions listed: \hyperlink{H__g}{${B_{H}}_{\mathrm{\mathrm{g}}}$}, the total gas used in the block, must be equal to the accumulated gas used according to the final transaction:
\begin{equation}
{B_{H}}_{\mathrm{\mathrm{g}}} = \hyperlink{ell}{\ell}(\hyperlink{R__u}{\mathbf{R})_{\mathrm{u}}}
\end{equation}
...
\begin{array}[b]{l}
\linkdest{Upsilon_pow_g2}\hyperlink{Upsilon_pow_g}{\Upsilon^{\mathrm{\mathrm{g}}}}(\boldsymbol{\sigma}[n - 1], B_{\mathbf{T}}[n])\\ \quad + \mathbf{R}[n-1]_{\mathrm{u}}
\end{array}
...
The mining proof-of-work (PoW) exists as a cryptographically secure nonce that proves beyond reasonable doubt that a particular amount of computation has been expended in the determination of some token value $\mathrm{n}$. It is utilised to enforce the blockchain security by giving meaning and credence to the notion of difficulty (and, by extension, total difficulty). However, since mining new blocks comes with an attached reward, the proof-of-work not only functions as a method of securing confidence that the blockchain will remain canonical into the future, but also as a wealth distribution mechanism.
...
We also assume a function $\mathrm{n}$, the trie's node cap function. When composing a node, we use RLP to encode the structure. As a means of reducing storage complexity, for nodes whose composed RLP is fewer than 32 bytes, we store the RLP directly; for those larger we assert prescience of the byte array whose Keccak hash evaluates to our reference. Thus we define in terms of $\mathrm{c}$, the node composition function:
\begin{equation}
...
A branch is then only used when necessary; no branch nodes may exist that contain only a single non-zero entry. We may formally define this structure with the structural composition function $\mathrm{c}$:
\begin{equation}
...
(\varnothing, 0, A^0, ()) & \text{if} \quad \mathrm{g} < g_{\mathrm{r}} \\
(\boldsymbol\sigma, \mathrm{g} - g_{\mathrm{r}}, A^0, \mathbf{o}) & \text{otherwise}\end{cases}
\end{equation}
...
\mathrm{\mathrm{g}}_{\mathrm{r}} &=& 3000\\
...
\mathrm{\mathrm{g}}_{\mathrm{r}} &=& 60 + 12\Big\lceil \dfrac{|I_{\mathbf{d}}|}{32} \Big\rceil\\
...
\end{equation}
for a natural number $\mathrm{n}$ and a point $P$ in $C_1$.

We define $P_1$ to be a point $(1,2)$ on $C_1$. Let $G_1$ be the subgroup of $(C_1,+)$ generated by $P_1$. $G_1$ is known to be a cyclic group of order $q$. For a point $P$ in $G_1$, we define $\log_{P_1}(P)$ to be the smallest natural number $\mathrm{n}$ satisfying $n\cdot P_1=P$. $\log_{P_1}(P)$ is at most $q-1$.
...
\end{eqnarray}
We define $G_2$ to be the subgroup of $(C_2,+)$ generated by $P_2$. $G_2$ is known to be a cyclic group of order $q$. For a point $P$ in $G_2$, we define $\log_{P_2}(P)$ be the smallest natural number $\mathrm{n}$ satisfying $n\cdot P_2=P$. With this definition, $\log_{P_2}(P)$ is at most $q-1$.
...
\qquad\Xi_{\mathtt{SNARKV}}(\boldsymbol\sigma,\mathrm{g},I)&=&(\varnothing,0,A^0,())\quad\text{if}\ F\\
...
\Xi_{\mathtt{BN\_ADD}}(\boldsymbol\sigma,\mathrm{g},I)&=&(\varnothing,0,A^0,())\quad\text{if}\ x=\varnothing\vee y=\varnothing\\
g_{\mathrm{r}} &=& 500\\
...
\Xi_{\mathtt{BN\_MUL}}(\boldsymbol\sigma,\mathrm{g},I)&=&(\varnothing,0,A^0,())\quad\text{if}\ x=\varnothing\\
g_{\mathrm{r}} &=& 40000\\
...
(T_{\mathrm{n}}, T_{\mathrm{p}}, T_{\mathrm{\mathrm{g}}}, T_{\mathrm{t}}, T_{\mathrm{v}}, \mathbf{p}) & \text{if} \; v \in \{27, 28\} \\
(T_{\mathrm{n}}, T_{\mathrm{p}}, T_{\mathrm{\mathrm{g}}}, T_{\mathrm{t}}, T_{\mathrm{v}}, \mathbf{p}, \mathtt{\tiny chain\_{\mathrm{id}}}, (), ()) & \text{otherwise} \\
\end{cases} \\
\nonumber \text{where} \\
...
&&&& The function $\mathrm{c}$ ensures the bytes default to zero if they extend past the limits.\\
...
&&&& with $\boldsymbol{c}(\boldsymbol{x}) \equiv (c(\boldsymbol{x}_0), ..., c(\boldsymbol{x}_{\lVert x \rVert -1})) $ with $\mathrm{c}$ as defined as above. \\
...
&&&& $(\boldsymbol{\sigma}', \boldsymbol{\mu}'_{\mathrm{\mathrm{g}}}, A^+, \mathbf{o}) \equiv \begin{cases}\Lambda(\boldsymbol{\sigma}^*, I_{\mathrm{a}}, I_{\mathrm{o}}, L(\boldsymbol{\mu}_{\mathrm{\mathrm{g}}}), I_{\mathrm{p}}, \boldsymbol{\mu}_{\mathbf{s}}[0], \mathbf{i}, I_{\mathrm{e}} + 1, I_{\mathrm{w}}) & \text{if} \quad \boldsymbol{\mu}_{\mathbf{s}}[0] \leqslant \boldsymbol{\sigma}[I_{\mathrm{a}}]_{\mathrm{b}} \;\wedge\; I_{\mathrm{e}} < 1024\\ \big(\boldsymbol{\sigma}, \boldsymbol{\mu}_{\mathrm{\mathrm{g}}}, \varnothing\big) & \text{otherwise} \end{cases}$ \\
...
&&&& $\boldsymbol{\mu}'_{\mathrm{\mathrm{g}}} \equiv \boldsymbol{\mu}_{\mathrm{\mathrm{g}}} + \mathrm{g}'$ \\
&&&& $\boldsymbol{\mu}'_{\mathbf{s}}[0] \equiv x$ \\
&&&& $A' \equiv A \Cup A^+$ \\
...
\min\{ L(\boldsymbol{\mu}_{\mathrm{\mathrm{g}}} - C_{\text{\tiny EXTRA}}(\boldsymbol{\sigma}, \boldsymbol{\mu})), \boldsymbol{\mu}_{\mathbf{s}}[0] \} & \text{if} \quad \boldsymbol{\mu}_{\mathrm{\mathrm{g}}} \ge C_{\text{\tiny EXTRA}}(\boldsymbol{\sigma}, \boldsymbol{\mu})\\
