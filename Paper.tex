...
\item[transactionsRoot] The Keccak 256-bit hash of the root node of the trie structure populated with each transaction in the transactions list portion of the block; formally $H_t$.\hypertarget{receipts_Root_def_words}{receipts Root def words}
...
\item[extraData] An arbitrary byte array containing data relevant to this block. This must be 32 bytes or fewer; formally $H_x$.\hypertarget{mixHash_H__m}{mixHash H m}
...
\end{equation}\hypertarget{Transaction Receipt}{Transaction Receipt}
...
Each receipt, denoted $B_\mathbf{R}[i]$ for the $i$th transaction, is placed in an index-keyed trie and the root recorded in the header as $H_e$.\hypertarget{transaction_receipt_R}{transaction receipt R}\hypertarget{tx_receipt_gas_used_R__u}{tx receipt gas used R u}
