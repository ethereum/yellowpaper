...
The values stemming from the computation of transactions, specifically the transaction receipts, $B_\mathbf{R}$, and that defined through the transaction's \hyperlink{Pi}{state-accumulation function, $\Pi$,} are formalised later in section \ref{sec:statenoncevalidation}.
...
B^* & \equiv & B \quad \text{except:} \quad B^*_\mathbf{r} = r(\hyperlink{Pi}{\Pi}(\Gamma(B), B))
\end{eqnarray}
