...
Note that $\mindifficulty$ is the difficulty of the genesis block. The \textit{\href{https://github.com/ethereum/EIPs/blob/master/EIPS/eip-606.md}{Homestead} difficulty parameter, $\homesteadmod$, is used to affect a dynamic homeostasis of time between blocks, as the time between blocks varies, as discussed below. $\expdiffsymb$ results in the difficulty increasing every 10000 blocks, which puts upward pressure on the time between blocks, causing it to slowly accelerate, and putting time pressure on transitioning to proof-of-stake. This effect was explained in EIP-649\cite{EIP-649} and implemented earlier in EIP-2\cite{EIP-2}.
