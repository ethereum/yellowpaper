...
\begin{equation}
\texttt{\small TRIE}\big(L_I^*(\boldsymbol{\sigma}[a]_\mathrm{s})\big) \equiv \boldsymbol{\sigma}[a]_s
\end{equation}
...
(T_n, T_p, T_g, T_t, T_v, T_\mathrm{i}, T_w, T_r, T_s) & \text{if} \; T_t = \varnothing\\
(T_n, T_p, T_g, T_t, T_v, T_\mathrm{d}, T_w, T_r, T_s) & \text{otherwise}
\end{cases}
\end{equation}
...
T_s \in \mathbb{P}_{256} & \wedge & T_\mathrm{d} \in \mathbb{B} & \wedge & T_\mathrm{i} \in \mathbb{B}
\end{array}
\end{equation}
where
\begin{equation}
...
\begin{equation}
T_\mathrm{t} \in \begin{cases} \mathbb{B}_{20} & \text{if} \quad T_t \neq \varnothing \\
...
The log entries, $R_\mathrm{l}$, is a series of log entries, termed, for example, $(O_0, O_1, ...)$. A log entry, $O$, is a tuple of a logger's address, $O_a$, a series of 32-bytes log topics, $O_\mathrm{t}$ and some number of bytes of data, $O_\mathrm{d}$:
\begin{equation}
O \equiv (O_a, ({O_\mathrm{t}}_0, {O_\mathrm{t}}_1, ...), O_\mathrm{d})
\end{equation}
\begin{equation}
O_a \in \mathbb{B}_{20} \quad \wedge \quad \forall_{t \in O_\mathrm{t}}: t \in \mathbb{B}_{32} \quad \wedge \quad O_\mathrm{d} \in \mathbb{B}
\end{equation}

We define the Bloom filter function, $M$, to reduce a log entry into a single 256-byte hash:
\begin{equation}
M(O) \equiv \bigvee_{t \in \{O_a\} \cup O_\mathrm{t}} \big( M_{3:2048}(t) \big)
\end{equation}
...
H_o &\equiv& \mathtt{\small KEC}(\mathtt{\small RLP}(L_H^*(B_\mathrm{U}))) & \wedge \\
H_t &\equiv& \mathtt{\small TRIE}(\{\forall i < \lVert B_\mathrm{T} \rVert, i \in \mathbb{P}: p(i, L_T(B_\mathrm{T}[i]))\}) & \wedge \\
H_e &\equiv& \mathtt{\small TRIE}(\{\forall i < \lVert B_\mathrm{R} \rVert, i \in \mathbb{P}: p(i, L_R(B_\mathrm{R}[i]))\}) & \wedge \\
H_b &\equiv& \bigvee_{\mathbf{r} \in B_\mathrm{R}} \big( \mathbf{r}_b \big)
\end{array}
\end{equation}
...
The values stemming from the computation of transactions, specifically the transaction receipts, $B_\mathrm{R}$, and that defined through the transactions state-accumulation function, $\Pi$, are formalised later in section \ref{sec:statenoncevalidation}.
...
\quad L_B(B) & \equiv & \big( L_H(B_H), L_T^*(B_\mathrm{T}), L_H^*(B_\mathrm{U}) \big)
\end{eqnarray}
...
\begin{equation*}
x \equiv \begin{cases}
1 & \text{if} \, \lVert P(H)_\mathrm{U}\rVert = 0 \\
2 & \text{otherwise}
\end{cases}
\end{equation*}
\begin{align}
...
\begin{align}
g_0 \equiv {} & \sum_{i \in T_\mathrm{i}, T_\mathrm{d}} \begin{cases} G_{txdatazero} & \text{if} \quad i = 0 \\ G_{txdatanonzero} & \text{otherwise} \end{cases} \\
...
T_g & \leqslant & {B_H}_l - \ell(B_\mathrm{R})_u
\end{array}
\end{equation}

Note the final condition; the sum of the transaction's gas limit, $T_g$, and the gas utilised in this block prior, given by $\ell(B_\mathrm{R})_u$, must be no greater than the block's \textbf{gasLimit}, ${B_H}_l$.
...
\Lambda_{4}(\boldsymbol{\sigma}_0, S(T), T_o, &\\ \quad\quad g, T_p, T_v, T_\mathrm{i}, 0, \top) & \text{if} \quad T_t = \varnothing \\
\Theta_{4}(\boldsymbol{\sigma}_0, S(T), T_o, &\\ \quad\quad T_t, T_t, g, T_p, T_v, T_v, T_\mathrm{d}, 0, \top) & \text{otherwise}
\end{cases}
\end{equation}
...
\forall i \in A_\mathrm{s}: \boldsymbol{\sigma}'[i] & = & \varnothing \\
\forall i \in A_\mathrm{t}: \boldsymbol{\sigma}'[i] & = & \varnothing \quad\text{if}\quad \mathtt{\tiny DEAD}(\boldsymbol{\sigma}^*\kern -2pt, i)
\end{eqnarray}
...
\end{cases} \\
\mathbf{a}^* &\equiv& (\boldsymbol{\sigma}[s]_n, \boldsymbol{\sigma}[s]_b - v, \boldsymbol{\sigma}[s]_\mathrm{s}, \boldsymbol{\sigma}[s]_c)
\end{eqnarray}
...
\end{equation}
\begin{equation}
\mathbf{a}_1 \equiv (\boldsymbol{\sigma}_1'[s]_n, \boldsymbol{\sigma}_1'[s]_b - v, \boldsymbol{\sigma}_1'[s]_\mathrm{s}, \boldsymbol{\sigma}_1'[s]_c)
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
\mathbf{a}_1' \equiv (\boldsymbol{\sigma}[r]_n, \boldsymbol{\sigma}[r]_b + v, \boldsymbol{\sigma}[r]_\mathrm{s}, \boldsymbol{\sigma}[r]_c)
\end{equation}
...
\mathbf{t} & \equiv & \{s, r\} \\
\\ \nonumber
\text{Let} \; \mathtt{\tiny KEC}(I_\mathrm{b}) & = & \boldsymbol{\sigma}[c]_c
\end{eqnarray}
...
\item $I_\mathrm{d}$, the byte array that is the input data to this execution; if the execution agent is a transaction, this would be the transaction data.
...
\boldsymbol{\mu}_\mathrm{o} & \equiv & ()
\end{eqnarray}
...
( w = \text{\small RETURNDATACOPY} \wedge \\ \quad \boldsymbol{\mu}_\mathrm{s}[1] + \boldsymbol{\mu}_\mathrm{s}[2] > \lVert\boldsymbol{\mu}_\mathrm{o}\rVert) \quad \vee \\
 \lVert\boldsymbol{\mu}_\mathrm{s}\rVert - \mathbf{\delta}_w + \mathbf{\alpha}_w > 1024 \quad \vee \\
...
w \in \{\text{\small CALL}, \text{\small CALLCODE}\} \wedge \boldsymbol{\mu}_\mathrm{s}[2] \neq 0
\end{array}
\end{equation}
...
\begin{equation}
\lVert B_\mathrm{U} \rVert \leqslant 2 \bigwedge_{U \in B_\mathrm{U}} V(U) \; \wedge \; k(U, P(B_H)_H, 6)
\end{equation}
...
\qquad\boldsymbol{\sigma}'[{B_H}_c]_b & = & \boldsymbol{\sigma}[{B_H}_c]_b + (1 + \frac{\lVert B_\mathrm{U}\rVert}{32})R_b \\
\qquad\forall_{U \in B_\mathrm{U}}: \\ \nonumber
\boldsymbol{\sigma}'[U_c] & = & \begin{cases}
\varnothing &\text{if}\ \boldsymbol{\sigma}[U_c] = \varnothing\ \wedge\ R = 0 \\
\mathbf{a}' &\text{otherwise}
\end{cases} \\
\mathbf{a}' &\equiv& (\boldsymbol{\sigma}[U_c]_n, \boldsymbol{\sigma}[U_c]_b + R, \boldsymbol{\sigma}[U_c]_\mathrm{s}, \boldsymbol{\sigma}[U_c]_c) \\
...
As previously detailed, $\mathbf{R}[n]_{s'}$, $\mathbf{R}[n]_\mathrm{l}$ and $\mathbf{R}[n]_u$ are the $n$th corresponding status code, logs and cumulative gas used after each transaction ($\mathbf{R}[n]_b$, the fourth component in the tuple, has already been defined in terms of the logs). We also define the $n$th state $\boldsymbol{\sigma}[n]$, which is defined simply as the state resulting from applying the corresponding transaction to the state resulting from the previous transaction (or the block's initial state in the case of the first such transaction):
\begin{equation}
\boldsymbol{\sigma}[n] = \begin{cases} \Gamma(B) & \text{if} \quad n < 0 \\ \Upsilon(\boldsymbol{\sigma}[n - 1], B_\mathrm{T}[n]) & \text{otherwise} \end{cases}
\end{equation}

In the case of $B_\mathrm{R}[n]_u$, we take a similar approach defining each item as the gas used in evaluating the corresponding transaction summed with the previous item (or zero, if it is the first), giving us a running total:
\begin{equation}
\mathbf{R}[n]_u = \begin{cases} 0 & \text{if} \quad n < 0 \\
\begin{array}[b]{l}
\Upsilon^g(\boldsymbol{\sigma}[n - 1], B_\mathrm{T}[n])\\ \quad + \mathbf{R}[n-1]_u
\end{array}
 & \text{otherwise} \end{cases}
\end{equation}

For $\mathbf{R}[n]_\mathrm{l}$, we utilise the $\Upsilon^\mathbf{l}$ function that we conveniently defined in the transaction execution function.
\begin{equation}
\mathbf{R}[n]_\mathrm{l} =
\Upsilon^\mathbf{l}(\boldsymbol{\sigma}[n - 1], B_\mathrm{T}[n])
\end{equation}

We define $\mathbf{R}[n]_{s'}$ in a similar manner.
\begin{equation}
\mathbf{R}[n]_{s'} =
\Upsilon^{s}(\boldsymbol{\sigma}[n - 1], B_\mathrm{T}[n])
\end{equation}
...
g_r &=& 60 + 12\Big\lceil \dfrac{|I_\mathrm{d}|}{32} \Big\rceil\\
\mathbf{o}[0..31] &=& \mathtt{\tiny SHA256}(I_\mathrm{d})\\
\Xi_{\mathtt{RIP160}} &\equiv& \Xi_{\mathtt{PRE}} \quad \text{where:} \\
g_r &=& 600 + 120\Big\lceil \dfrac{|I_\mathrm{d}|}{32} \Big\rceil\\
\mathbf{o}[0..11] &=& 0 \\
...
g_r &=& 15 + 3\Big\lceil \dfrac{|I_\mathrm{d}|}{32} \Big\rceil\\
...
F&\equiv&(|I_\mathrm{d}|\bmod 192\neq 0\vee(\exists j.\ a_j=\varnothing\vee b_j=\varnothing))\\
k &=& \dfrac{|I_\mathrm{d}|}{192} \\
g_r&=& 60000k + 40000 \\
...
G_{exp} & \text{if} \quad w = \text{\small EXP} \wedge \boldsymbol{\mu}_\mathrm{s}[1] = 0 \\
G_{exp} + G_{expbyte}\times(1+\lfloor\log_{256}(\boldsymbol{\mu}_\mathrm{s}[1])\rfloor) & \text{if} \quad w = \text{\small EXP} \wedge \boldsymbol{\mu}_\mathrm{s}[1] > 0 \\
G_{verylow} + G_{copy}\times\lceil\boldsymbol{\mu}_\mathrm{s}[2] \div 32\rceil & \text{if} \quad w = \text{\small CALLDATACOPY} \lor \text{\small CODECOPY} \lor \text{\small RETURNDATACOPY} \\
G_{extcode} + G_{copy}\times\lceil\boldsymbol{\mu}_\mathrm{s}[3] \div 32\rceil & \text{if} \quad w = \text{\small EXTCODECOPY} \\
G_{log}+G_{logdata}\times\boldsymbol{\mu}_\mathrm{s}[1] & \text{if} \quad w = \text{\small LOG0} \\
G_{log}+G_{logdata}\times\boldsymbol{\mu}_\mathrm{s}[1]+G_{logtopic} & \text{if} \quad w = \text{\small LOG1} \\
G_{log}+G_{logdata}\times\boldsymbol{\mu}_\mathrm{s}[1]+2G_{logtopic} & \text{if} \quad w = \text{\small LOG2} \\
G_{log}+G_{logdata}\times\boldsymbol{\mu}_\mathrm{s}[1]+3G_{logtopic} & \text{if} \quad w = \text{\small LOG3} \\
G_{log}+G_{logdata}\times\boldsymbol{\mu}_\mathrm{s}[1]+4G_{logtopic} & \text{if} \quad w = \text{\small LOG4} \\
...
&&&& $ \forall i \in [0..255]: \boldsymbol{\mu}'_\mathrm{s}[0]_i \equiv \begin{cases} \boldsymbol{\mu}_\mathrm{s}[1]_t &\text{if} \quad i \leqslant t \quad \text{where} \; t = 256 - 8(\boldsymbol{\mu}_\mathrm{s}[0] + 1) \\ \boldsymbol{\mu}_\mathrm{s}[1]_i &\text{otherwise} \end{cases}$ \\
\multicolumn{5}{l}{$\boldsymbol{\mu}_\mathrm{s}[x]_i$ gives the $i$th bit (counting from zero) of $\boldsymbol{\mu}_\mathrm{s}[x]$} \vspace{5pt} \\
\midrule
\end{tabular*}
...
&&&& $\forall i \in [0..255]: \boldsymbol{\mu}'_\mathrm{s}[0]_i \equiv \boldsymbol{\mu}_\mathrm{s}[0]_i \wedge \boldsymbol{\mu}_\mathrm{s}[1]_i$ \\
\midrule
0x17 & {\small OR} & 2 & 1 & Bitwise OR operation. \\
&&&& $\forall i \in [0..255]: \boldsymbol{\mu}'_\mathrm{s}[0]_i \equiv \boldsymbol{\mu}_\mathrm{s}[0]_i \vee \boldsymbol{\mu}_\mathrm{s}[1]_i$ \\
\midrule
0x18 & {\small XOR} & 2 & 1 & Bitwise XOR operation. \\
&&&& $\forall i \in [0..255]: \boldsymbol{\mu}'_\mathrm{s}[0]_i \equiv \boldsymbol{\mu}_\mathrm{s}[0]_i \oplus \boldsymbol{\mu}_\mathrm{s}[1]_i$ \\
\midrule
0x19 & {\small NOT} & 1 & 1 & Bitwise NOT operation. \\
&&&& $\forall i \in [0..255]: \boldsymbol{\mu}'_\mathrm{s}[0]_i \equiv \begin{cases} 1 & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[0]_i = 0 \\ 0 & \text{otherwise} \end{cases}$ \\
\midrule
0x1a & {\small BYTE} & 2 & 1 & Retrieve single byte from word. \\
&&&& $\forall i \in [0..255]: \boldsymbol{\mu}'_\mathrm{s}[0]_i \equiv \begin{cases} \boldsymbol{\mu}_\mathrm{s}[1]_{(i + 8\boldsymbol{\mu}_\mathrm{s}[0])} & \text{if} \quad i < 8 \wedge \boldsymbol{\mu}_\mathrm{s}[0] < 32 \\ 0 & \text{otherwise} \end{cases} $\\
...
&&&& $\boldsymbol{\mu}'_i \equiv M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[0], \boldsymbol{\mu}_\mathrm{s}[1])$ \\
\bottomrule
\end{tabular*}
...
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv I_a$ \\
\midrule
0x31 & {\small BALANCE} & 1 & 1 & Get balance of the given account. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv \begin{cases}\boldsymbol{\sigma}[\boldsymbol{\mu}_\mathrm{s}[0]]_b& \text{if} \quad \boldsymbol{\sigma}[\boldsymbol{\mu}_\mathrm{s}[0] \mod 2^{160}] \neq \varnothing\\0&\text{otherwise}\end{cases}$ \\
\midrule
0x32 & {\small ORIGIN} & 0 & 1 & Get execution origination address. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv I_o$ \\
&&&& This is the sender of original transaction; it is never an account with non-empty \\
&&&& associated code. \\
\midrule
0x33 & {\small CALLER} & 0 & 1 & Get caller address. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv I_s$ \\
...
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv I_v$ \\
\midrule
...
&&&& $\boldsymbol{\mu}'_i \equiv M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[0], \boldsymbol{\mu}_\mathrm{s}[2])$ \\
&&&& This pertains to the input data passed with the message call instruction or transaction. \\
\midrule
...
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv I_p$ \\
&&&& This is gas price specified by the originating transaction.\\
\midrule
0x3b & {\small EXTCODESIZE} & 1 & 1 & Get size of an account's code. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv \lVert \boldsymbol{\sigma}[\boldsymbol{\mu}_\mathrm{s}[0] \mod 2^{160}]_c \rVert$ \\
\midrule
...
&&&& where $\mathbf{c} \equiv \boldsymbol{\sigma}[\boldsymbol{\mu}_\mathrm{s}[0] \mod 2^{160}]_c$ \\
&&&& $\boldsymbol{\mu}'_i \equiv M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[1], \boldsymbol{\mu}_\mathrm{s}[3])$ \\
...
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv \lVert \boldsymbol{\mu}_\mathrm{o} \rVert$ \\
\midrule
0x3e & {\small RETURNDATACOPY} & 3 & 0 & Copy output data from the previous call to memory. \\
&&&& $\forall_{i \in \{ 0 \dots \boldsymbol{\mu}_\mathrm{s}[2] - 1\} } \boldsymbol{\mu}'_\mathrm{m}[\boldsymbol{\mu}_\mathrm{s}[0] + i ] \equiv
\begin{cases} \boldsymbol{\mu}_\mathrm{o}[\boldsymbol{\mu}_\mathrm{s}[1] + i] & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[1] + i < \lVert \boldsymbol{\mu}_\mathrm{o} \rVert \\ 0 & \text{otherwise} \end{cases}$\\
&&&& The additions in $\boldsymbol{\mu}_\mathrm{s}[1] + i$ are not subject to the $2^{256}$ modulo. \\
&&&& $\boldsymbol{\mu}'_i \equiv M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[0], \boldsymbol{\mu}_\mathrm{s}[2])$ \\
\bottomrule
\end{tabular*}
...
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv P(I_{H_p}, \boldsymbol{\mu}_\mathrm{s}[0], 0)$ \\
...
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv {I_H}_c$ \\
\midrule
0x42 & {\small TIMESTAMP} & 0 & 1 & Get the block's timestamp. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv {I_H}_s$ \\
\midrule
0x43 & {\small NUMBER} & 0 & 1 & Get the block's number. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv {I_H}_i$ \\
\midrule
0x44 & {\small DIFFICULTY} & 0 & 1 & Get the block's difficulty. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv {I_H}_d$ \\
\midrule
0x45 & {\small GASLIMIT} & 0 & 1 & Get the block's gas limit. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv {I_H}_l$ \\
\bottomrule
\end{tabular*}
...
&&&& $\boldsymbol{\mu}'_i \equiv \max(\boldsymbol{\mu}_i, \ceil{ (\boldsymbol{\mu}_\mathrm{s}[0] + 32) \div 32 })$ \\
...
&&&& $\boldsymbol{\mu}'_i \equiv \max(\boldsymbol{\mu}_i, \ceil{ (\boldsymbol{\mu}_\mathrm{s}[0] + 1) \div 32 })$ \\
&&&& The addition in the calculation of $\boldsymbol{\mu}'_i$ is not subject to the $2^{256}$ modulo. \\
\midrule
0x54 & {\small SLOAD} & 1 & 1 & Load word from storage. \\
&&&& $\boldsymbol{\mu}'_\mathrm{s}[0] \equiv \boldsymbol{\sigma}[I_a]_\mathrm{s}[\boldsymbol{\mu}_\mathrm{s}[0]]$ \\
\midrule
0x55 & {\small SSTORE} & 2 & 0 & Save word to storage. \\
&&&& $\boldsymbol{\sigma}'[I_a]_\mathrm{s}[ \boldsymbol{\mu}_\mathrm{s}[0] ] \equiv \boldsymbol{\mu}_\mathrm{s}[1] $ \\
&&&& $C_{\text{\tiny SSTORE}}(\boldsymbol{\sigma}, \boldsymbol{\mu}) \equiv \begin{cases}
G_{sset} & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[1] \neq 0 \; \wedge \; \boldsymbol{\sigma}[I_a]_\mathrm{s}[\boldsymbol{\mu}_\mathrm{s}[0]] = 0 \\
G_{sreset} & \text{otherwise}
\end{cases}$ \\
&&&& $A'_{r} \equiv A_{r} + \begin{cases}
R_{sclear} & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[1] = 0 \; \wedge \; \boldsymbol{\sigma}[I_a]_\mathrm{s}[\boldsymbol{\mu}_\mathrm{s}[0]] \neq 0 \\
0 & \text{otherwise}
\end{cases}$ \\
\midrule
...
\multicolumn{5}{l}{$A'_\mathrm{l} \equiv A_\mathrm{l} \cdot (I_a, \mathbf{t}, \boldsymbol{\mu}_\mathrm{m}[ \boldsymbol{\mu}_\mathrm{s}[0] \dots (\boldsymbol{\mu}_\mathrm{s}[0] + \boldsymbol{\mu}_\mathrm{s}[1] - 1) ])$}\\
\multicolumn{5}{l}{and to update the memory consumption counter:}\\
\multicolumn{5}{l}{$\boldsymbol{\mu}'_i \equiv M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[0], \boldsymbol{\mu}_\mathrm{s}[1])$}\\
...
&&&& $(\boldsymbol{\sigma}', \boldsymbol{\mu}'_g, A^+, \mathbf{o}) \equiv \begin{cases}\Lambda(\boldsymbol{\sigma}^*, I_a, I_o, L(\boldsymbol{\mu}_g), I_p, \boldsymbol{\mu}_\mathrm{s}[0], \mathbf{i}, I_e + 1, I_w) & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[0] \leqslant \boldsymbol{\sigma}[I_a]_b \;\wedge\; I_e < 1024\\ \big(\boldsymbol{\sigma}, \boldsymbol{\mu}_g, \varnothing\big) & \text{otherwise} \end{cases}$ \\
&&&& $\boldsymbol{\sigma}^* \equiv \boldsymbol{\sigma} \quad \text{except} \quad \boldsymbol{\sigma}^*[I_a]_n = \boldsymbol{\sigma}[I_a]_n + 1$ \\
&&&& $A' \equiv A \Cup A^+$ which abbreviates: $A'_\mathrm{s} \equiv A_\mathrm{s} \cup A^+_\mathrm{s} \quad \wedge \quad A'_\mathrm{l} \equiv A_\mathrm{l} \cdot A^+_\mathrm{l} \quad \wedge \quad A'_\mathrm{t} \equiv A_\mathrm{t} \cup A^+_\mathrm{t}$ \\
&&&& $ \wedge \quad A'_\mathrm{r} \equiv A_\mathrm{r} + A^+_\mathrm{r}$ \\
...
&&&& (the maximum call depth limit is reached) or $\boldsymbol{\mu}_\mathrm{s}[0] > \boldsymbol{\sigma}[I_a]_b$ (balance of the caller is too \\
&&&& low to fulfil the value transfer); and otherwise $x=A(I_a, \boldsymbol{\sigma}[I_a]_n)$, the address of the newly \\
&&&& created account. \\
&&&& $\boldsymbol{\mu}'_i \equiv M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[1], \boldsymbol{\mu}_\mathrm{s}[2])$ \\
&&&& $\boldsymbol{\mu}'_\mathrm{o} \equiv ()$ \\
...
&&&& $(\boldsymbol{\sigma}', g', A^+, \mathbf{o}) \equiv \begin{cases}\begin{array}{l}\Theta(\boldsymbol{\sigma}, I_a, I_o, t, t,\\ \quad C_{\text{\tiny CALLGAS}}(\boldsymbol{\mu}), I_p, \boldsymbol{\mu}_\mathrm{s}[2], \boldsymbol{\mu}_\mathrm{s}[2], \mathbf{i}, I_e + 1, I_w)\end{array} & \begin{array}{l}\text{if} \quad \boldsymbol{\mu}_\mathrm{s}[2] \leqslant \boldsymbol{\sigma}[I_a]_b \;\wedge \\ \quad\quad I_e < 1024\end{array}\\ (\boldsymbol{\sigma}, g, \varnothing, ()) & \text{otherwise} \end{cases}$ \\
...
&&&& $\boldsymbol{\mu}'_\mathrm{o} = \mathbf{o}$ \\
...
&&&& $\boldsymbol{\mu}_\mathrm{s}[2] > \boldsymbol{\sigma}[I_a]_b$ (not enough funds) or $I_e = 1024$ (call depth limit reached); $x=1$ \\
&&&& otherwise. \\
&&&& $\boldsymbol{\mu}'_i \equiv M(M(\boldsymbol{\mu}_i, \boldsymbol{\mu}_\mathrm{s}[3], \boldsymbol{\mu}_\mathrm{s}[4]), \boldsymbol{\mu}_\mathrm{s}[5], \boldsymbol{\mu}_\mathrm{s}[6])$ \\
...
C_{\text{\tiny GASCAP}}(\boldsymbol{\sigma}, \boldsymbol{\mu}) + G_{callstipend} & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[2] \neq 0 \\
...
G_{callvalue} & \text{if} \quad \boldsymbol{\mu}_\mathrm{s}[2] \neq 0 \\
0 & \text{otherwise}
\end{cases}$ \\
&&&& $C_{\text{\tiny NEW}}(\boldsymbol{\sigma}, \boldsymbol{\mu}) \equiv \begin{cases}
G_{newaccount} & \text{if} \quad \mathtt{\tiny DEAD}(\boldsymbol{\sigma}, \boldsymbol{\mu}_\mathrm{s}[1] \mod 2^{160}) \wedge \boldsymbol{\mu}_\mathrm{s}[2] \neq 0 \\
0 & \text{otherwise}
\end{cases}$ \\
\midrule
...
&&&& argument is $\boldsymbol{\mu}_\mathrm{s}[2]$. As a result, $\boldsymbol{\mu}_\mathrm{s}[3]$, $\boldsymbol{\mu}_\mathrm{s}[4]$, $\boldsymbol{\mu}_\mathrm{s}[5]$ and $\boldsymbol{\mu}_\mathrm{s}[6]$ in the definition of {\small CALL} \\
&&&& should respectively be replaced with $\boldsymbol{\mu}_\mathrm{s}[2]$, $\boldsymbol{\mu}_\mathrm{s}[3]$, $\boldsymbol{\mu}_\mathrm{s}[4]$ and $\boldsymbol{\mu}_\mathrm{s}[5]$. \\
&&&& Otherwise exactly equivalent to {\small CALL} except: \\
&&&& $(\boldsymbol{\sigma}', g', A^+, \mathbf{o}) \equiv \begin{cases}\begin{array}{l}\Theta(\boldsymbol{\sigma}^*, I_s, I_o, I_a, t,\\\quad \boldsymbol{\mu}_\mathrm{s}[0], I_p, 0, I_v, \mathbf{i}, I_e + 1, I_w)\end{array} & \text{if} \quad I_v \leqslant \boldsymbol{\sigma}[I_a]_b \;\wedge\; I_e < 1024 \\ (\boldsymbol{\sigma}, g, \varnothing, ()) & \text{otherwise} \end{cases}$ \\
...
&&&& Exactly equivalent to {\small CALL} except: \\
&&&& The argument $\boldsymbol{\mu}_\mathrm{s}[2]$ is replaced with $0$. \\
&&&& The deeper argument $\boldsymbol{\mu}_\mathrm{s}[3]$, $\boldsymbol{\mu}_\mathrm{s}[4]$, $\boldsymbol{\mu}_\mathrm{s}[5]$ and $\boldsymbol{\mu}_\mathrm{s}[6]$ are respectively replaced with \\
&&&& $\boldsymbol{\mu}_\mathrm{s}[2]$, $\boldsymbol{\mu}_\mathrm{s}[3]$, $\boldsymbol{\mu}_\mathrm{s}[4]$ and $\boldsymbol{\mu}_\mathrm{s}[5]$. \\
...
&&&& $A'_\mathrm{s} \equiv A_\mathrm{s} \cup \{ I_a \}$ \\
&&&& $\boldsymbol{\sigma}'[r] \equiv \begin{cases}
\varnothing &\text{if}\ \boldsymbol{\sigma}[r] = \varnothing\ \wedge\ \boldsymbol{\sigma}[I_a]_b = 0\\
(\boldsymbol{\sigma}[r]_n, \boldsymbol{\sigma}[r]_b + \boldsymbol{\sigma}[I_a]_b, \boldsymbol{\sigma}[r]_\mathrm{s}, \boldsymbol{\sigma}[r]_c) & \text{if}\ r \neq I_a \\
(\boldsymbol{\sigma}[r]_n, 0, \boldsymbol{\sigma}[r]_\mathrm{s}, \boldsymbol{\sigma}[r]_c) & \text{otherwise}
\end{cases}$\\
&&&& where $r = \boldsymbol{\mu}_\mathrm{s}[0] \bmod 2^{160}$\\
...
0 & \text{otherwise}
\end{cases}$ \\
&&&& $n \equiv \mathtt{\tiny DEAD}(\boldsymbol{\sigma}, \boldsymbol{\mu}_\mathrm{s}[0] \mod 2^{160}) \wedge \boldsymbol{\sigma}[I_a]_b \neq 0$ \\
\bottomrule
\end{tabular*}
