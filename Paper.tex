...
\begin{equation}
Z(\mathbb{S}, \mathbb{M}, I) \equiv \mathbb{M}_g < C(\mathbb{S}, \mathbb{M}, I) \vee \lVert\mathbb{M}_T\rVert < \mathbf{\delta}_w
\end{equation}
...
The application of rewards to a block involves little more 
...
\begin{equation}
C(\mathbb{S}, \mathbb{M}) \equiv C_M(\mathbb{S}, \mathbb{M}) + \begin{cases}
...
&&&& $\mathbb{M}'_T[0] \equiv \mathbb{S}'[I_a]_{storage}[\mathbb{M}_T[0]]$ \\
\midrule
0x57 & {\small SSTORE} & 2 & 0 & Save word to storage. \\
&&&& $\mathbb{S}'[I_a]_{storage}[ \mathbb{M}_T[0] ] \equiv \mathbb{M}_T[1] $ \\
&&&& $c_{\text{\tiny SSTORE}} \equiv \begin{cases}
2G_{sstore} & \text{if} \quad \mathbb{M}_T[1] \neq 0 \; \wedge \; \mathbb{S}[I_a]_{storage}[\mathbb{M}_T[0]] = 0 \\
0 & \text{if} \quad \mathbb{M}_T[1] = 0 \; \wedge \; \mathbb{S}[I_a]_{storage}[\mathbb{M}_T[0]] \neq 0 \\
G_{sstore} & \text{otherwise}
\end{cases}$ \\
\midrule
0x58 & {\small JUMP} & 1 & 0 & Alter the program counter. \\
&&&& $J_{\text{\tiny JUMP}}(\mathbb{M}) \equiv \mathbb{M}_T[0] $ \\
...
&&&& $J_{\text{\tiny JUMPI}}(\mathbb{M}) \equiv \begin{cases} \mathbb{M}_T[0] & \text{if} \quad \mathbb{M}_T[0] = 0 \\ \mathbb{M}_{pc} + 1 & \text{otherwise} \end{cases} $ \\
&&&& This has the effect of writing said value to $\mathbb{M}_{pc}$. See section \ref{ch:model}. \\
\midrule
...
&&&& $H_{\text{\tiny RETURN}}(\mathbb{M}) \equiv \mathbb{M}_M[ \mathbb{M}_T[0] \dots ( \mathbb{M}_T[0] + \mathbb{M}_T[1] - 1 ) ]$ \\
