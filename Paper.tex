...
There are a number of different types of payload that may be encoded within the RLP. This ''type'' is always determined by the first entry of the RLP, represented as a scalar:
...
0x00 & {\small Hello} & {\small Hello}? & $(\text{0x}00, v \in \mathbb{P}, n \in \mathbb{P}, \mathbf{i} \in \mathbb{B}, c \in \mathbb{P}, p \in \mathbb{P}, i \in \mathbb{B}_{64})$ \\
\midrule
0x01 & {\small Disconnect} & {\small Disconnect}? & $(\text{0x}01, r \in \mathbb{P})$ \\
\midrule
0x02 & {\small Ping} & {\small Pong} & $(\text{0x}02)$ \\
\midrule
0x03 & {\small Pong} && $(\text{0x}03)$ \\
\midrule
...
\begin{tabular*}{\columnwidth}[h]{rlll}
\toprule
\multicolumn{4}{c}{\textbf{00s: Information}} \vspace{5pt} \\
\textbf{Value} & \textbf{Mnemonic} & \textbf{Expected Reply} & \textbf{Packet Format} \vspace{5pt} \\
0x10 & {\small GetPeers} & {\small Peers} & $(\text{0x}10)$ \\
\midrule
0x11 & {\small Peers} & & $(\text{0x}11, (a_1 \in \mathbb{B}_4, a_1 \in \mathbb{P}), (), ...)$ \\
\midrule
0x12 & {\small Transactions} & {\small Pong} & $(\text{0x}12)$ \\
\midrule
0x13 & {\small Blocks} && $(\text{0x}13)$ \\
\midrule
0x14 & {\small GetChain} & {\small Chain} & $(\text{0x}14)$ \\
\midrule
0x15 & {\small NotInChain} && $(\text{0x}15)$ \\
\midrule
0x16 & {\small GetTransactions} & {\small Transactions} & $(\text{0x}16)$ \\
\bottomrule
\end{tabular*}
...
$\mathtt{\tiny SHA3}\big( (42) \big)$ refers to the SHA3 hash of a byte array of length one whose first and only byte is of value 42.
