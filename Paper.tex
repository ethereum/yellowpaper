...
where
\begin{equation}
\mathbb{B}_{\mathrm{n}} = \{ B: B \in \mathbb{B} \wedge \lVert B \rVert = \mathrm{n} \}
\end{equation}
...
\begin{equation}
n \leqslant \frac{2^{256}}{H_{\mathrm{d}}} \quad \wedge \quad \mathrm{m} = H_{\mathrm{m}}
\end{equation}
with $(n, \mathrm{m}) = \mathtt{PoW}(H_{\hcancel{n}}, H_{\mathrm{n}}, \mathbf{d})$.
...
\begin{eqnarray}
V(H) & \equiv &  \mathrm{n} \leqslant \frac{2^{256}}{H_{\mathrm{d}}} \wedge \mathrm{m} = H_{\mathrm{m}} \quad \wedge \\
...
\end{eqnarray}
where $(n, \mathrm{m}) = \mathtt{PoW}(H_{\hcancel{n}}, H_{\mathrm{n}}, \mathbf{d})$
...
Evaluating $\boldsymbol{\sigma}_{P}$ from $\boldsymbol{\sigma}_0$ depends on the transaction type; either contract creation or message call; we define the tuple of post-execution provisional state $\boldsymbol{\sigma}_{P}$, remaining gas $\mathrm{g}'$, substate $A$ and status code $z$:
\begin{equation}
(\boldsymbol{\sigma}_{P}, \mathrm{g}', A, \mathrm{z}) \equiv \begin{cases}
...
\Upsilon^{\mathrm{z}}(\boldsymbol{\sigma}, T) & \equiv & \mathrm{z}
\end{eqnarray}
...
\begin{equation}
(\boldsymbol{\sigma}', \mathrm{g}', A, \mathrm{z}, \mathbf{o}) \equiv \Lambda(\boldsymbol{\sigma}, s, o, \mathrm{g}, p, v, \mathbf{i}, \mathrm{e}, w)
\end{equation}
...
If there is not enough gas remaining to pay this, \ie $\mathrm{g}^{**} < \mathrm{c}$, then we also declare an out-of-gas exception.
...
If such an exception does not occur, then the remaining gas is refunded to the originator and the now-altered state is allowed to persist. Thus formally, we may specify the resultant state, gas, substate and status code as $(\boldsymbol{\sigma}', \mathrm{g}', A, \mathrm{z})$ where:

\begin{align}
\quad \mathrm{g}' &\equiv \begin{cases}
0 & \text{if} \quad F \\
\mathrm{g}^{**} - \mathrm{c} & \text{otherwise} \\
\end{cases} \\
...
\end{cases} \\
\quad \mathrm{z} &\equiv \begin{cases}
0 & \text{if} \quad \boldsymbol{\sigma}^{**} = \varnothing \lor \mathrm{g}^{**} < \mathrm{c} \\
1 & \text{otherwise}
\end{cases} \\
\nonumber \text{where} \\
F &\equiv \big((\boldsymbol{\sigma}^{**} = \varnothing \ \wedge\ \mathbf{o} = \varnothing) \vee\  \mathrm{g}^{**} < \mathrm{c} \ \vee\  |\mathbf{o}| > 24576\big)
\end{align}
...
\begin{equation}
(\boldsymbol{\sigma}', \mathrm{g}', A, \mathrm{z}, \mathbf{o}) \equiv \Theta(\boldsymbol{\sigma}, s, o, r, \mathrm{c}, \mathrm{g}, p, v, \tilde{v}, \mathbf{d}, \mathrm{e}, w)
\end{equation}
...
(a, b, \mathrm{c}, d) \cdot \mathrm{e} & \equiv & (a, b, \mathrm{c}, d, \mathrm{e}) \\
...
\begin{equation}
k(U, H, \mathrm{n}) \equiv \begin{cases} false & \text{if} \quad \mathrm{n} = 0 \\
s(U, H) &\\
\quad \vee \; k(U, P(H)_{H}, \mathrm{n} - 1) & \text{otherwise}
\end{cases}
\end{equation}
...
B'_{\mathrm{n}} & = & \mathrm{n}: \quad x \leqslant \frac{2^{256}}{\hyperlink{H__d}{H_{\mathrm{d}}}} \\
B'_{\mathrm{m}} & = & \mathrm{m} \quad \text{with } (x, \mathrm{m}) = \mathtt{PoW}(B^*_{\hyperlink{H_cancel_n}{\hcancel{n}}}, \mathrm{n}, \mathbf{d}) \\
...
\begin{equation}
\boldsymbol{\sigma}[n] = \begin{cases} \hyperlink{Gamma}{\Gamma}(B) & \text{if} \quad \mathrm{n} < 0 \\ \hyperlink{Upsilon}{\Upsilon}(\boldsymbol{\sigma}[n - 1], B_{\mathbf{T}}[n]) & \text{otherwise} \end{cases}
\end{equation}
...
\begin{equation}
\mathbf{R}[n]_{\mathrm{u}} = \begin{cases} 0 & \text{if} \quad \mathrm{n} < 0 \\
\begin{array}[b]{l}
...
\begin{equation}
m = H_{\mathrm{m}} \quad \wedge \quad \mathrm{n} \leqslant \frac{2^{256}}{H_{\mathrm{d}}} \quad \text{with} \quad (m, \mathrm{n}) = \mathtt{PoW}(H_{\hcancel{n}}, H_{\mathrm{n}}, \mathbf{d})
\end{equation}
...
\end{cases} \\
\mathtt{\tiny BE}(x) & \equiv & (b_0, b_1, ...): b_0 \neq 0 \wedge x = \sum_{n = 0}^{n < \lVert \mathbf{b} \rVert} b_{\mathrm{n}} \cdot 256^{\lVert \mathbf{b} \rVert - 1 - \mathrm{n}} \\
(a) \cdot (b, \mathrm{c}) \cdot (d, \mathrm{e}) & = & (a, b, \mathrm{c}, d, \mathrm{e})
\end{eqnarray}
...
c(\mathfrak{I}, i) & \text{if} \quad \lVert \mathrm{c}(\mathfrak{I}, i)\rVert < 32 \\
...
\begin{equation}
L(n) \equiv \mathrm{n} - \lfloor \mathrm{n} / 64 \rfloor
\end{equation}
...
&&&& $P(h, \mathrm{n}, a) \equiv \begin{cases} 0 & \text{if} \quad \mathrm{n} > H_{\mathrm{i}} \vee a = 256 \vee \mathrm{h} = 0 \\ \mathrm{h} & \text{if} \quad \mathrm{n} = H_{\mathrm{i}} \\ P(H_{\mathrm{p}}, \mathrm{n}, a + 1) & \text{otherwise} \end{cases}$ \\
...
&&&& $\boldsymbol{\mu}'_{\mathbf{s}}[0] \equiv \mathrm{c}(\boldsymbol{\mu}_{pc} + 1)$ \\
&&&& $\text{where} \quad \mathrm{c}(x) \equiv \begin{cases} I_{\mathbf{b}}[x] & \text{if} \quad x < \lVert I_{\mathbf{b}} \rVert \\ 0 & \text{otherwise} \end{cases}$ \\
...
&&&& with $\boldsymbol{c}(\boldsymbol{x}) \equiv (c(\boldsymbol{x}_0), ..., \mathrm{c}(\boldsymbol{x}_{\lVert x \rVert -1})) $ with $c$ as defined as above. \\
...
&&&& $\boldsymbol{\mu}'_{\mathbf{m}}[ \boldsymbol{\mu}_{\mathbf{s}}[5] \dots (\boldsymbol{\mu}_{\mathbf{s}}[5] + \mathrm{n} - 1) ] = \mathbf{o}[0 \dots (n - 1)]$ \\
...
G_{newaccount} & \text{if} \quad \mathrm{n} \\
0 & \text{otherwise}
\end{cases}$ \\
...
\begin{equation}
 \mathbf{c'}[i] = \mathbf{c}_{i} \quad \forall \quad i < \mathrm{n}
\end{equation}
\begin{equation}
 \mathrm{n} = \left\lfloor\frac{c_{size}}{J_{hashbytes}}\right\rfloor
\end{equation}
...
\begin{equation}
 E_\text{\tiny RMH}(\mathbf{x}) = \big( E_{rmh}(\mathbf{x}, 0), E_{rmh}(\mathbf{x}, 1), ... , E_{rmh}(\mathbf{x}, \mathrm{n} - 1) \big)\linkdest{E__cacherounds}{}
\end{equation}
\begin{multline}
  E_{rmh}(\mathbf{x}, i) = \texttt{KEC512}(\mathbf{x'}[(i - 1 + \mathrm{n}) \mod \mathrm{n}] \oplus \mathbf{x'}[\mathbf{x'}[i][0] \mod \mathrm{n}]) \\
