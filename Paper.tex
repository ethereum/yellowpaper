...
\title{Ethereum: A Secure Decentralised Generalised Transaction Ledger \\ {\smaller \textbf{Homestead Draft}}}
\author{
    Dr. Gavin Wood\\
...
Since the system is decentralised and all parties have an opportunity to create a new block on some older pre-existing block, the resultant structure is necessarily a tree of blocks. In order to form a consensus as to which path (known as blockchain) from root (the genesis block) to leaf (the block containing the most recent transactions) through this tree structure is to be considered canonical, there must be an agreed-upon scheme. If there is ever a disagreement between nodes as to which root-to-leaf path down the block tree is the `best' blockchain, then a \textit{fork} occurs.
...
We define the function $\texttt{\small TRIE}$, which evaluates to the root of the trie that represents this set when encoded in this structure (see also formal definition \ref{eq:structuralcomposition} for the structural composition function $c$):
\begin{equation}
...
\begin{equation} \label{eq:structuralcomposition}
