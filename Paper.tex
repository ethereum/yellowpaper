...
Let $G_T$ be the multiplicative abelian group underlying $F_{p^{12}}$. It is known that a non-degenerate bilinear map $e : G_1\times G_2 \to G_T$ exists.
This bilinear map is a type three pairing. 
...
We define $\Xi_{\mathtt{SNARKV}}$ as a precompiled contract for zkSNARK verification. $\Xi_{\mathtt{SNARKV}}$ takes a set of points $a_i$ that are in $G_1$, and a set of points $b_i$ that are in $G_2$. It checks that
\begin{equation}
\prod_{i=0}^{n}e\left(a_i, b_i\right) = 1
\end{equation}
If this is true, the contract returns $1$, otherwise it returns $0$, unless any of the points $a_i$ or $b_i$ are not on their respective curves, in which case the transaction is reverted.
