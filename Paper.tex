...
The size for Ethash's cache $\mathbf{c} \in \mathbb{B}$  and dataset $\mathbf{d} \in \mathbb{B}$ depend on the epoch, which depends on the block number.
\begin{equation}
...
\subsubsection{Cache}
The cache production process involves using the seed hash to first sequentially filling up $c_{size}$ bytes of memory, then performing $J_{cacherounds}$ passes of the RandMemoHash algorithm created by \cite{lerner2014randmemohash}:
...
E_{cacherounds}(E_{rmh}(\mathbf{x}), y -1 ) & \text{otherwise}
\end{cases}
\end{equation}
...
E_\text{\tiny FNV}(\mathbf{m}, \mathbf{c}[(E_\text{\tiny FNV}(i \oplus p), \mathbf{m}[ \lfloor J_{hashbytes} / J_{wordbytes} \rfloor]) \mod c_{size}) & \text{otherwise}
\end{cases}
\end{equation}
